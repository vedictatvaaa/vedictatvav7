...
ALTER TABLE master_services DROP CONSTRAINT IF EXISTS master_services_booking_mode_valid;
...
ALTER TABLE puja_bookings DROP CONSTRAINT IF EXISTS puja_bookings_travel_amount_nonnegative;
...
ALTER TABLE puja_bookings DROP CONSTRAINT IF EXISTS puja_bookings_distance_nonnegative;
...
ALTER TABLE puja_bookings DROP CONSTRAINT IF EXISTS puja_bookings_total_amount_nonnegative;
ALTER TABLE puja_bookings ADD CONSTRAINT puja_bookings_total_amount_nonnegative CHECK (total_amount >= 0);
...
CREATE OR REPLACE FUNCTION reject_overlapping_active_travel_band() RETURNS trigger AS $$
BEGIN
  IF NEW.is_active AND EXISTS (
    SELECT 1 FROM travel_bands existing
    WHERE existing.is_active AND existing.id <> COALESCE(NEW.id, 0)
      AND NEW.min_distance_km <= existing.max_distance_km
      AND NEW.max_distance_km >= existing.min_distance_km
  ) THEN
    RAISE EXCEPTION 'Active travel bands cannot overlap' USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS travel_bands_no_active_overlap ON travel_bands;
CREATE TRIGGER travel_bands_no_active_overlap
  BEFORE INSERT OR UPDATE OF min_distance_km, max_distance_km, is_active ON travel_bands
  FOR EACH ROW EXECUTE FUNCTION reject_overlapping_active_travel_band();
...
CREATE TABLE IF NOT EXISTS puja_booking_events (
  id integer GENERATED ALWAYS AS IDENTITY PRIMARY KEY, booking_id integer NOT NULL REFERENCES puja_bookings(id),
  event_type text NOT NULL, recipient_party text NOT NULL, recipient_id integer, event_key text, payload jsonb, created_at timestamp NOT NULL DEFAULT now()
);
ALTER TABLE puja_booking_events ADD COLUMN IF NOT EXISTS event_key text;
UPDATE puja_booking_events SET event_key = concat('legacy:', id) WHERE event_key IS NULL;
ALTER TABLE puja_booking_events ALTER COLUMN event_key SET NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS puja_booking_events_event_key_unique ON puja_booking_events(event_key);
...
CREATE TABLE IF NOT EXISTS puja_booking_deliveries (
  ...
  created_at timestamp NOT NULL DEFAULT now(), updated_at timestamp NOT NULL DEFAULT now(),
   ...
   CONSTRAINT puja_booking_deliveries_attempt_count_nonnegative CHECK (attempt_count >= 0)
);
