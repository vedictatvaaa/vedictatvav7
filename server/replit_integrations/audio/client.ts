...
export function getAudioAiClient() {
  return createAiClient({ task: "audio", model: "gpt-audio" });
}
...
export async function voiceChat(
  ...
  const response = await getAudioAiClient().chat.completions.create({
    model: "gpt-audio",
...
export async function voiceChatStream(
  ...
  const audioBase64 = audioBuffer.toString("base64");
  const stream = await getAudioAiClient().chat.completions.create({
    model: "gpt-audio",
    modalities: ["text", "audio"],
...
export async function speechToText(
  ...
  const response = await getAudioAiClient().audio.transcriptions.create({
    file,
...
export async function speechToTextStream(
  ...
  const file = await toFile(audioBuffer, `audio.${format}`);
  const stream = await getAudioAiClient().audio.transcriptions.create({
    file,
    model: "gpt-4o-mini-transcribe",
    stream: true,
  });
