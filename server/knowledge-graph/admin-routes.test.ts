...
function response() {
  const out: any = { statusCode: 200, body: undefined, ended: false, status(code: number) { out.statusCode = code; return out; }, json(body: any) { out.body = body; return out; }, end() { out.ended = true; return out; } };
  return out;
}
...
test("all graph endpoints are guarded and relationship/rule audit actors come only from req.adminUserId", async () => {
  ...
  assert.equal(app.routes.length, 14);
