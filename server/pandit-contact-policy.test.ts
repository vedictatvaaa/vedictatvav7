...
test("every global and override combination obeys the disabled ceiling", () => {
  const cases: Array<[string, string, string]> = [
    ["open", "use_global", "login_required"], ["open", "always_open", "login_required"], ["open", "login_required", "login_required"], ["open", "never_display", "disabled"],
    ["login_required", "use_global", "login_required"], ["login_required", "always_open", "login_required"], ["login_required", "login_required", "login_required"], ["login_required", "never_display", "disabled"],
...
test("rolling quota metadata cannot become negative and reset follows earliest reveal", () => {
  ...
  assert.equal(state.resetsAt, "2025-07-01T00:00:00.000Z");
});