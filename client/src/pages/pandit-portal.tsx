...
export default function PanditPortalPage() {
  ...
  async function changePassword() { if (password.length < 8) { toast({ title: "Use at least 8 characters", variant: "destructive" }); return; } try { await panditApi("POST", "/api/pandit/change-password", { newPassword: password }); toast({ title: "Password updated" }); setPasswordChangeRequired(false); setPwdOpen(false); setPassword(""); } catch (e: any) { toast({ title: "Could not update password", description: e.message, variant: "destructive" }); } }
  ...
  function content() {
    ...
    if (section === "services") return <PanditStorefrontEditor focus="services" />; if (section === "gallery") return <FoundationUnavailable title="Gallery" detail="Gallery data is not available in this practice system yet." />;
    ...
    if (section === "japa") return <div className="min-w-0 space-y-5"><PanditSectionHeader title="Jap Counter" description="Track your daily sadhana and keep your mantra practice moving." /><JapCounter ownerKey={`pandit:${data.me?.id || "self"}`} devoteeName={data.me?.name} embedded /></div>;
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#f6f0e4] text-[#35231d]">
    ...
    {mobileOpen && <button className="fixed inset-0 z-30 bg-[#35231d]/30 lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation overlay" />}
     <div className="min-w-0 lg:pl-72"><header className="sticky top-0 z-20 flex h-16 items-center gap-2 border-b border-[#d8c8ae] bg-[#fffaf1]/90 px-3 backdrop-blur md:gap-3 md:px-8"><button className="rounded-lg p-2 hover:bg-[#f2e6d2] lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation" data-testid="button-mobile-menu"><Menu className="h-5 w-5" /></button><div className="flex min-w-0 items-center gap-2 lg:hidden"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-[#55252d] font-serif text-sm text-[#e6b957]">V</span><span className="truncate font-serif text-sm font-semibold text-[#55252d]">Vedic Tatva</span></div><div className="min-w-0 flex-1"><p className="hidden text-[10px] font-bold uppercase tracking-[.2em] text-[#946c16] sm:block">Practice headquarters</p><h1 className="truncate font-serif text-base font-semibold text-[#55252d] md:text-xl">{active.label}</h1></div><div className="flex shrink-0 items-center gap-1.5"><PanditPwaInstallButton /><button onClick={() => go("notifications")} className="relative rounded-lg p-2 text-[#665445] hover:bg-[#f2e6d2]" aria-label="Notifications" data-testid="button-mobile-notifications"><Bell className="h-4 w-4" />{data.summary?.today?.unreadMessages?.state === "available" && data.summary.today.unreadMessages.count > 0 && <span className="absolute right-0.5 top-0.5 min-w-4 rounded-full bg-[#55252d] px-1 text-center text-[9px] font-bold text-[#fff8e9]">{data.summary.today.unreadMessages.count}</span>}</button><button onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-[#665445] hover:bg-[#f2e6d2] lg:hidden" aria-label={`Open ${data.me?.name || "profile"} menu`} data-testid="button-mobile-profile"><Users className="h-4 w-4" /></button><StoreActions path={data.summary?.storefront?.publicPath || null} /></div></header><main className="mx-auto min-w-0 max-w-[1440px] px-4 py-5 pb-24 md:px-8 md:py-8 md:pb-8">{data.error && <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">We couldn't load your practice. <button className="font-semibold underline" onClick={data.refresh}>Try again</button></div>}{data.loading ? <Loading /> : content()}</main></div>
      <MobileNav active={section} go={go} onMore={() => setMobileOpen(true)} unread={data.summary?.today?.unreadMessages?.state === "available" ? data.summary.today.unreadMessages.count : 0} /><Dialog open={pwdOpen} onOpenChange={open => { if (!passwordChangeRequired || open) setPwdOpen(open); }}><DialogContent className="max-w-sm" onEscapeKeyDown={event => { if (passwordChangeRequired) event.preventDefault(); }} onPointerDownOutside={event => { if (passwordChangeRequired) event.preventDefault(); }}><DialogHeader><DialogTitle>{passwordChangeRequired ? "Create your new password" : "Update your password"}</DialogTitle><DialogDescription>{passwordChangeRequired ? "For security, replace the temporary password before continuing." : "Choose a password with at least 8 characters."}</DialogDescription></DialogHeader><Input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="New password" /><Button onClick={changePassword} className="bg-[#55252d] text-[#fff8e9] hover:bg-[#3e1b20]">Update password</Button></DialogContent></Dialog>
  </div>;
}
...
function FoundationUnavailable({ title, detail }: { title: string; detail: string }) { return <div className="space-y-5"><PanditSectionHeader title={title} description={detail} /><PanditUnavailableState title={`${title} is not connected`} detail={detail} action={<span className="inline-flex rounded-full bg-[#f2e6d2] px-3 py-1 text-xs font-semibold text-[#665445]">Setup required</span>} /></div>; }
