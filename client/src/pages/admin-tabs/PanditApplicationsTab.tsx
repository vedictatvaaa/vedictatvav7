...
function PanditApplicationsTab({ adminToken }: { adminToken?: string }) {
  ...
  const decisionMutation = useMutation({
    mutationFn: async ({ id, action, note }: { id: number; action: "approve" | "reject"; note: string }) => {
      ...
      return { ...body, action } as { success: boolean; action: "approve" | "reject"; message?: string; registrationNo?: string; approvalEmailSent?: boolean; application?: PanditApplication };
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/pandit-applications"] });
      ...
      toast({
        ...
        description: data.action === "approve"
          ? `A Pandit account has been created${data.registrationNo ? ` (registration no. ${data.registrationNo})` : ""}.${data.approvalEmailSent ? " A secure account-setup link was emailed." : " The setup email could not be queued; use Send password reset from the Pandits tab."}`
