...
function NotificationsTab({ adminToken }: { adminToken?: string }) {
  ...
  return (
    <div className="space-y-6" data-testid="notifications-tab">
      ...
      <UnifiedAlertOperations adminToken={adminToken} />
...
interface UnifiedAlertOverview {
  externalDeliveryEnabled: boolean;
  deliveries: { total: number; queued: number; processing: number; sent: number; skipped: number; retrying: number; failed: number };
  events: { total: number; pending: number };
  preferences: { total: number; panchangEnabled: number };
  recent: Array<{
    id: number;
    eventKey: string;
    eventType: string;
...
function UnifiedAlertOperations({ adminToken }: { adminToken?: string }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const fetcher = createFetcher(adminToken);
  const overview = useQuery<UnifiedAlertOverview>({
    queryKey: ["/api/admin/alerts/overview"],
    queryFn: () => fetcher("/api/admin/alerts/overview"),
  });
  ...
  const processQueue = async () => {
    try {
      ...
      if (!response.ok) throw new Error(data?.message || "Queue processing failed");
      toast({ title: "Alert queue processed", description: `${data.result?.processed || 0} delivery rows handled.` });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/alerts/overview"] });
    } catch (error: any) {
      toast({ title: "Could not process alert queue", description: error?.message, variant: "destructive" });
    }
  };
  ...
  const counts = data.deliveries;
  const statItems = [
    ["Queued", counts.queued, "text-amber-700"],
    ["Sent", counts.sent, "text-emerald-700"],
    ["Skipped", counts.skipped, "text-slate-600"],
    ["Failed", counts.failed, "text-red-700"],
  ] as const;

  return (
    <Card data-testid="unified-alert-operations">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <CardTitle className="text-base flex items-center gap-2"><Zap className="w-4 h-4" /> Unified alert operations</CardTitle>
            <CardDescription>
              {data.events.total} durable event{data.events.total === 1 ? "" : "s"} · {data.preferences.panchangEnabled}/{data.preferences.total} users with Panchang alerts enabled.
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <span className={`text-xs rounded-full px-2 py-1 border ${data.externalDeliveryEnabled ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-50 text-amber-800"}`}>
              External channels {data.externalDeliveryEnabled ? "enabled" : "safety-off"}
            </span>
            <Button size="sm" variant="outline" onClick={processQueue}>
              <RefreshCw className="w-3.5 h-3.5 mr-1" /> Process queue
            </Button>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {statItems.map(([label, value, color]) => (
            <div key={label} className="rounded-md border border-muted bg-muted/30 px-3 py-2">
              ...
              <tbody>
                {data.recent.slice(0, 8).map((row) => (
                  <tr key={row.id} className="border-t">
                    ...
                    <td className="px-3 py-2">
                      <span className={`rounded-full px-2 py-0.5 ${row.status === "sent" ? "bg-emerald-50 text-emerald-800" : row.status === "failed" ? "bg-red-50 text-red-800" : "bg-slate-100 text-slate-700"}`}>
                        {row.status}
                      </span>
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">{new Date(row.scheduledFor).toLocaleString("en-IN")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
