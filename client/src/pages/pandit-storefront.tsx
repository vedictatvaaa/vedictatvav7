...
type StorefrontDto = {
  ...
  services?: Service[];
  packages?: Array<{ id: number; name: string; description?: string; price?: number; compareAtPrice?: number; durationMinutes?: number; masterServiceIds?: number[]; items?: Array<{ panditServiceId: number; masterServiceId?: number; displayOrder?: number }>; }>;
...
function bookingHref(
  pandit: StorefrontDto["pandit"],
  service?: Service,
  packageSelection?: { id: number; masterServiceIds?: number[] },
) {
  ...
  if (packageSelection) {
    ...
    params.set("packageId", String(packageSelection.id));
    const masterServiceIds = (packageSelection.masterServiceIds || []).filter(id => Number.isInteger(id) && id > 0);
    if (masterServiceIds.length === 1) params.set("masterServiceId", String(masterServiceIds[0]));
    else params.delete("masterServiceId");
  }
...
export default function PanditStorefrontPage() {
  ...
  const book = (service?: Service, packageSelection?: { id: number; masterServiceIds?: number[] }) => {
    ...
    trackPanditSeoEvent("discovery_cta", { slug, source: "storefront" }); trackPanditSeoEvent("booking_handoff", { slug, source: "storefront" }); navigate(bookingHref(pandit, service, packageSelection));
  };
  const chat = () => {
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#FCF8F0] pb-20 text-[#422C29] md:pb-0">
    ...
    <main className="mx-auto max-w-[1220px] px-3 sm:px-5 lg:px-7">
      ...
      {!!data.packages?.length && <section className="border-t border-[#E9DCC6] py-7"><SectionTitle kicker="Curated combinations" title="Puja packages" /><div className="grid gap-3 sm:grid-cols-2">{data.packages.map(pkg => <Card key={pkg.id} className="rounded-xl border-[#E9DCC6] bg-[#FFFDF9] p-4"><div className="flex justify-between gap-3"><div><h3 className="font-semibold text-[#531D28]">{pkg.name}</h3><p className="mt-1 text-xs leading-5 text-[#735E54]">{pkg.description}</p></div><Package className="h-5 w-5 text-[#9A641F]" /></div><div className="mt-4 flex items-center justify-between border-t border-[#EEE3D2] pt-3"><strong className="text-[#8D2830]">{money(pkg.price)}</strong><Button onClick={() => book(undefined, pkg)} className="h-8 rounded-md bg-[#8D2830] text-xs">Book package</Button></div></Card>)}</div></section>}
    ...
    <PanditSharePanel open={shareOpen} onOpenChange={setShareOpen} storefrontUrl={shareUrl} storyImageUrl={`/api/story/p/${encodeURIComponent(slug)}.jpg`} previewImageUrl={`/api/og/p/${encodeURIComponent(slug)}.jpg`} panditName={displayName} source="storefront" storyPreviewAlt={`Share Story preview for ${displayName}`} returnFocusRef={shareTriggerRef} />
