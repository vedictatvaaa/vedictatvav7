...
export default function PanditStorefrontPage() {
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#FBF4E3] text-[#4D312A]">
    <PageSeo title={`${pandit.name} — Verified Vedic Pandit | Vedic Tatva`} description={storefront?.tagline || storefront?.bio || pandit.bio || `Book a Vedic ceremony with ${pandit.name}.`} canonical={`/pandit/${slug}`} ogType="profile" ogImage={pandit.image || storefront?.bannerImage || undefined} />
    ...
    <main>
      <section className="relative isolate overflow-hidden border-b border-[#D5AE59]/25 bg-[#7C291F] text-[#FFF8E8]">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-8 px-5 py-14 sm:py-20 lg:grid-cols-[1.05fr_.95fr]">
          <div className="order-2 lg:order-1"><div className="mb-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.3em] text-[#F4D47F]"><CheckCircle2 className="h-4 w-4" />Verified Vedic Pandit</div><h1 className="max-w-xl text-5xl leading-[.98] sm:text-7xl">{storefront?.tagline || "Rituals held with reverence."}</h1><p className="mt-6 max-w-lg text-base leading-7 text-[#FBE9C9]/80">A personal space for sacred ceremonies, thoughtful guidance, and bookings handled with care.</p><div className="mt-8 flex flex-wrap gap-3"><Button onClick={book} className="h-12 rounded-none bg-[#F2B544] px-6 text-[#5B1D27] hover:bg-[#F7D889]"><CalendarDays className="mr-2 h-4 w-4" />Book a Puja</Button><Button variant="outline" onClick={chat} className="h-12 rounded-none border-[#FBE9C9]/45 bg-transparent px-6 text-[#FFF8E8] hover:bg-[#FFF8E8]/10"><MessageCircle className="mr-2 h-4 w-4" />Chat privately</Button></div><div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-xs text-[#FBE9C9]/75">{pandit.city && <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4 text-[#F2B544]" />{pandit.city}{pandit.state ? `, ${pandit.state}` : ""}</span>}{languages.length > 0 && <span className="inline-flex items-center gap-2"><Languages className="h-4 w-4 text-[#F2B544]" />{languages.join(" · ")}</span>}{pandit.experience ? <span>{pandit.experience}+ years of practice</span> : null}</div></div>
      ...
      {!!reviews.length && <section id="reviews" className="mx-auto max-w-6xl px-5 py-16 sm:py-24"><SectionHeading eyebrow="Words from devotees" title="A trusted presence for family ceremonies" /><div className="grid gap-5 md:grid-cols-3">{reviews.slice(0, 6).map(review => <Card key={review.id} className="rounded-none border-[#D5AE59]/35 bg-[#FFFDF7] p-5"><div className="flex gap-1">{[1,2,3,4,5].map(i => <Star key={i} className={`h-4 w-4 ${i <= review.rating ? "fill-[#C98B20] text-[#C98B20]" : "text-[#D8CDB5]"}`} />)}</div><p className="mt-4 text-sm leading-6 text-[#6D5A50]">“{review.comment}”</p><div className="mt-5 text-xs font-bold text-[#7C291F]">{review.userName || review.reviewerName || "Devotee"}{review.serviceType ? <span className="font-normal text-[#806E62]"> · {review.serviceType}</span> : null}</div></Card>)}</div></section>}
